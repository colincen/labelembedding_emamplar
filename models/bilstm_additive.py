...
class Bilstm_LabelEmbedding(nn.Module):
    def __init__(self, config, embedding, word2Idx, label2Idx, char2Idx, description, exemplar):
        ...
        if self.use_charEmbedding:
            self.charemb = charEmbedding(eval(config.conv_filter_sizes), eval(config.conv_filter_nums), char2Idx,
                                         config.char_emb_size, self.device)
        ...
        # self.W1 = nn.Linear(2 * self.hidden_size if self.Bidirectional else self.hidden_size,
        #                               self.dim3,
        #                               bias=True)
        # self.W2 = nn.Linear(self.embed_size, self.dim3, bias=False)
        self.W1 = nn.Parameter(torch.empty(2 * self.hidden_size if self.Bidirectional else self.hidden_size,self.dim3))
        self.W2 =nn.Parameter(torch.empty(1206, self.dim3))
        self.dropout = nn.Dropout(self.dropout_rate)
        self.v = nn.Parameter(torch.empty(self.dim3))
        nn.init.uniform_(self.v, -0.5, 0.5)
        nn.init.uniform(self.W1, -0.5, 0.5)
        nn.init.uniform(self.W2, -0.5, 0.5)
    ...
    def forward(self, x, y, Type):
        ...
        atten_left = torch.matmul(x, self.W1)
        atten_left = torch.unsqueeze(atten_left,-2)

        atten_left = atten_left.repeat([1, 1, self.LabelEmbedding.size(0), 1])
        atten_right = torch.matmul(self.LabelEmbedding, self.W2)
        atten_right = torch.unsqueeze(atten_right, 0).unsqueeze(0)
        atten = atten_left + atten_right

        # print(self.v.size())
        y_hat = torch.tanh(atten).matmul(self.v)
        # y_hat = torch.tanh(atten).matmul(self.v)
        # print(y_hat.size())
        ...
        # atten_left = torch.matmul(x, self.W1)
        # atten_right = torch.matmul(self.LabelEmbedding, self.W2)
        # print(atten_left.size())
        # print(atten_right.size())
        # y_hat = torch.tanh(atten_left + atten_right).dot(self.v)
        # print(y_hat)
        # print(x.size())
        #
        # print(self.LabelEmbedding.size())
        #
        # print(self.W1.weight.size())
        # print(self.W2.weight.size())
        # print(self.v.size())

...
# def cal_maxlen(da):
#     return max([len(x) for x in da])
#
# def padData(da, max_len, padding_idx):
#     resda = copy.deepcopy(da)
#     for i in range(len(resda)):
#         while len(resda[i]) < max_len:
#             resda[i].append(padding_idx)
#     return resda
...
if __name__ == '__main__':
    ...
    for pa in data_generator(da, lab, batch_size=2):
